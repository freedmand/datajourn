        ...
        if (input[cursor] === quoteChar) {
          ...
          for (;;) {
            ...
            if (input[quoteSearch + 1 + spacesBetweenQuoteAndDelimiter] === delim) {
              row.push(
                input.substring(cursor, quoteSearch).replace(quoteCharRegex, quoteChar)
              );
              cursor = quoteSearch + 1 + spacesBetweenQuoteAndDelimiter + delimLen;

              // If char after following delimiter is not quoteChar, we find next quote char position
              if (
                input[quoteSearch + 1 + spacesBetweenQuoteAndDelimiter + delimLen] !==
                quoteChar
              ) {
...
function d(value, fallback) {
  return value === undefined ? fallback : value;
}

function processCSV(file, config, complete, callback = null) {
  // Config variables
  const sendProgress = d(config.sendProgress, true);
  const progressIncrement = d(config.progressIncrement, 1000);
  const progressStart = d(config.progressStart, 0);
  const progressEnd = d(config.progressEnd, 1);
  const excludeColumns = d(config.excludeColumns, []);
  const excludeSpec = {};
  for (let i = 0; i < excludeColumns.length; i++) {
    excludeSpec[excludeColumns[i]] = true;
  }
  let rowStart = d(config.rowStart, 0);
  let rowEnd = d(config.rowEnd, null);
  const survey = d(config.survey, false);
  const surveyCutoff = d(config.surveyCutoff, 1000);
  const cache = d(config.cache, []);
  let firstRow = d(config.firstRow, []);

  let slicingFile = false;
  let fileRowSkip = 0;
  if (rowStart > 0 && cache.length > 0) {
    let bestIdx = null;
    for (let i = 0; i < cache.length; i++) {
      const [rowSkip, fileSeek] = cache[i];
      if (rowSkip > rowStart) {
        break;
      } else {
        bestIdx = [rowSkip, fileSeek];
      }
    }
    if (bestIdx != null) {
      const [rowSkip, fileSeek] = bestIdx;
      fileRowSkip = rowSkip;
      slicingFile = true;
      file = file.slice(fileSeek);
      rowStart -= rowSkip;
      if (rowEnd != null) rowEnd -= rowSkip;
    }
  }

  const prepopulatedFirstRow = firstRow.length != 0 && slicingFile;

  const size = file.size;

  // Variables that will get populated
  let headers = null;
  let columns = null;
  const results = [];
  ...
  // Populated in survey to enable fast skips later
  const cacheIndices = [];
  ...
  Papa.parse(file, {
    ...
    complete: function() {
      callback({
        ...
        rowsRead,
        totalRows,
        start: rowStart + 1 + fileRowSkip,
        end: rowStart + rowsRead + fileRowSkip,
        cacheIndices,
        firstRow,
      });
    },
    step: function(row, parser) {
      if (first) {
        ...
        const rd = prepopulatedFirstRow ? firstRow.slice() : row.data;

        columns = rd.length;
        firstRow = rd.slice();
        for (let i = 0; i < columns; i++) {
          if (excludeSpec[i]) continue;

          headers.push(rd[i].trim());

          // Populate start of data structure
          let initialData = {
            data: [],
            codes: [], // unique listing of data
            cellToCode: {}, // map cell to code
            uniques: {}, // unique counts
            idx: 0, // index of uniques
            numUniques: 0, // number of uniques
            numBlanks: 0, // number of blanks encountered
            numNumbers: 0, // number of numbers encountered
            count: 0, // total values encountered
            countAll: 0, // total values encountered including skips
        ...
        if (!prepopulatedFirstRow) {
          return;
        }
      }

      if (row.data.length < columns) {
        // Skip shorter rows with no data
        let valid = false;
      ...
      for (let i = 0; i < columns; i++) {
        ...
        const cell = row.data[i] == null ? '' : row.data[i];
        ...
        if (cell.trim() == '') {
          ...
          if (num === +cell) {
            ...
            if (result.maxNumber == null || num > result.maxNumber) {
              result.maxNumber = num;
            }
            if (result.minNumber == null || num < result.minNumber) {
              result.minNumber = num;
            }
            if (num == 0) {
              // Zero
              result.zeroNumbers++;
      ...
      if (skip > 0) {
        ...
        for (let i = 0; i < columns; i++) {
          ...
          if (result.cellToCode[cell] == null) {
            // Newly encountered data
            result.codes.push(cell);
            result.cellToCode[cell] = result.idx;
            result.data.push(result.idx);
            result.idx++;
            result.uniques[cell] = 1;
            result.numUniques++;
          } else {
            // Already encountered data
            result.data.push(result.cellToCode[cell]);
            result.uniques[cell]++;
          }
          result.count++;
        }
      ...
      if (rowEnd != null && totalRows >= rowEnd) {
        // Check for exceeding row end
        parser.abort();
      }

      if (sendProgress || survey) {
        // Check for progress
        const cursor = row.meta.cursor;

        if (survey) {
          // Calculate new skip value if surveying
          const expectedTotalRows = totalRows * (size / cursor);
          const totalRowsRemaining = expectedTotalRows - totalRows;
          const cutoffRowsRemaining = surveyCutoff - rowsRead;
          skip = totalRowsRemaining / cutoffRowsRemaining;
        }

        if (sendProgress) {
          let newProgress =
            rowEnd != null
              ? Math.round((totalRows / rowEnd) * progressIncrement)
              : Math.round((cursor / size) * progressIncrement);
          if (survey) {
            // If surveying, progress can be higher based on survey cutoff
            newProgress = Math.max(
              newProgress,
              Math.round((rowsRead / surveyCutoff) * progressIncrement)
            );
          }
          if (newProgress != progress) {
            if (survey) {
              // Push cache index
              cacheIndices.push([totalRows, cursor]);
            }
            progress = newProgress;
            const p = progress / progressIncrement;
            postMessage({
              type: `progress-${complete}`,
              progress: (p * (progressEnd - progressStart) + progressStart) * 100,
            });
          }
        }
      }
...
onmessage = function(e) {
  processCSV(e.data.file, e.data.config, e.data.complete, results => {
    postMessage({type: e.data.complete, results});
  });
};
